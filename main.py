...
alzheimer_images, alzheimer_labels = None, None
test_images = None
...
# Function to load the dataset of a person with Alzheimer's disease
def load_alzheimer_dataset():
    global alzheimer_images, alzheimer_labels
    folder_path = filedialog.askdirectory(title="Select Alzheimer's Dataset Folder")
    if folder_path:
        alzheimer_images, alzheimer_labels = load_images_from_directory(folder_path, label=1)
        if check_dataset(alzheimer_images, "Alzheimer's"):
            status_label.config(text="Alzheimer's dataset loaded successfully.")
            result_text.delete(1.0, tk.END)  # Clear previous results
        else:
            status_label.config(text="Invalid Alzheimer's dataset. Please check the data.")
...
def display_test_results(predictions):
    ...
    for i, (prediction, filename) in enumerate(
        ...
        label = tk.Label(result_frame, text=f"Image {filename}\nPrediction: {'Alzheimers Disease' if prediction >= 0.5 else 'Healthy'}")
        label.pack()
...
def train_and_test():
    global healthy_images, healthy_labels, alzheimer_images, alzheimer_labels, test_images

    if healthy_images is None or alzheimer_images is None or test_images is None:
        result_text.delete(1.0, tk.END)  # Clear previous results
        status_label.config(text="Please load all datasets before training and testing.")
        return

    # Combine datasets
    X = np.concatenate((healthy_images, alzheimer_images), axis=0)
    y = np.concatenate((healthy_labels, alzheimer_labels), axis=0)

    # Print information about the loaded datasets
    status_label.config(
        text=f"Total number of samples: {len(X)}\nNumber of healthy samples: {np.sum(y == 0)}\nNumber of Alzheimer's samples: {np.sum(y == 1)}")
    ...
    for i, (prediction, filename) in enumerate(
            zip(predictions, os.listdir('test'))):
        result_text.insert(tk.END, f"Image {filename}: {'Alzheimers Disease' if prediction >= 0.5 else 'Healthy'}\n")
...
# Buttons to load healthy and Alzheimer's datasets
...
load_alzheimer_button = tk.Button(button_frame, text="Load Alzheimer's Dataset", command=load_alzheimer_dataset)
load_alzheimer_button.pack(side=tk.LEFT, padx=10)
